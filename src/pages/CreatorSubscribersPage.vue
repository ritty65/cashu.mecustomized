...
function onExportCsv() {
  downloadSubscribersCsv(filtered.value, `subscribers_${new Date().toISOString().slice(0,10)}.csv`);
}
...
<template>
  <div class="q-pa-md">
    ...
    >
      <div class="text-bold">
        {{ missingEndpoint ? 'Set VITE_SUBSCRIBERS_ENDPOINT or localStorage.creator_npub' : store.error }}
      </div>
      <template #action>
    ...
    <div class="row items-center q-gutter-md q-mb-md">
      ...
      />
      <q-input dense filled debounce="200" v-model="store.search" placeholder="Search subscribers..." class="col-grow" />
      <q-btn dense outline icon="refresh" @click="store.fetchSubscribers(true)" />
      <q-btn dense outline icon="download" label="Export CSV" @click="onExportCsv" />
    </div>

    <!-- KPI + Charts -->
    <div class="q-mb-md">
      <!-- Replace with your actual KPI row/cards -->
    ...
    <div v-else>
      ...
      <div v-else class="row q-col-gutter-md">
        <div v-for="s in filtered" :key="s.id || s.npub" class="col-12 col-sm-6 col-md-4">
          <subscriber-card :subscriber="s" @click="store.select(s.id || s.npub)" />
        </div>
      </div>
    </div>
