...
export const verifyP2PKSig = (proof) => {
    if (!proof.witness) {
    ...
    //     }
    // }
    return schnorr.verify(proof.witness.signatures[0], sha256(new TextDecoder().decode(proof.secret)), parsedSecret[1].data);
};
export const verifyP2PKSigOutput = (output, publicKey) => {
    if (!output.witness) {
        throw new Error('could not verify signature, no witness provided');
    }
