...
export const createP2PKsecret = (pubkey) => {
    const newSecret = [
        'P2PK',
        {
...
export const signP2PKsecret = (secret, privateKey) => {
    const msghash = sha256(new TextDecoder().decode(secret));
    const sig = schnorr.sign(msghash, privateKey);
    return sig;
};
export const signBlindedMessage = (B_, privateKey) => {
    const msgHash = sha256(B_);
    const sig = schnorr.sign(msgHash, privateKey);
    return sig;
};
export const getSignedProofs = (proofs, privateKey) => {
    let keypairs = [];
    let pk = '';
    ...
    return proofs.map((p) => {
        try {
            const parsed = parseSecret(p.secret);
            if (parsed[0] !== 'P2PK') {
...
export const getSignedOutput = (output, privateKey) => {
...
export const getSignedOutputs = (outputs, privateKey) => {
    return outputs.map((o) => getSignedOutput(o, privateKey));
};
export const getSignedProof = (proof, privateKey) => {
    if (!proof.witness) {
        proof.witness = {
