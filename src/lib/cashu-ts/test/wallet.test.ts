...
describe("receive", () => {
	...
	test("test receive tokens already spent", async () => {
		...
		server.use(
			http.post(mintUrl + "/v1/swap", () => {
				return new HttpResponse(JSON.stringify({ detail: msg }), {
					status: 400,
				});
			}),
		);
		...
		expect(result).toEqual(
			new Error("tokens already spent. Secret: asdasdasd"),
		);
	});
		...
		server.use(
			http.post(mintUrl + "/v1/swap", () => {
				return new HttpResponse(
					JSON.stringify({ code: 0, error: "could not verify proofs." }),
					{
						status: 400,
					},
				);
	...
	const proofs = [
		{
			...
			amount: 1,
			secret:
				"1f98e6837a434644c9411825d7c6d6e13974b931f8f0652217cea29010674a13",
...
describe("deterministic", () => {
	test("no seed", async () => {
		...
		const result = await wallet
			...
			.catch((e) => e);
		expect(result).toEqual(
			new Error("cannot create deterministic messages without seed"),
		);
	});
	test.each([
		[
		...
		"deterministic OutputData: counter %i -> secret: %s, r: %s",
		...
		const data = OutputData.createP2PKData(
			{
				pubkey: "thisisatest",
				locktime: 212,
				rfndKeys: ["iamarefund", "asecondrefund"],
			},
			21,
			keys,
		);
...
describe("Restoring deterministic proofs", () => {
	test("Batch restore", async () => {
		...
		const mockRestore = vi
			...
			.mockImplementation(
				async (start, count, options?): Promise<{ proofs: Array<Proof> }> => {
					if (rounds === 0) {
						rounds++;
	...
	test("Batch restore with custom values", async () => {
			...
			.mockImplementation(
				async (
					...
					if (rounds === 0) {
						rounds++;
						return {
							proofs: Array(42).fill(1) as Array<Proof>,
							lastCounterWithSignature: 41,
						};
					}
					rounds++;
					return { proofs: [] };
		...
		const { proofs: restoredProofs, lastCounterWithSignature } =
			await wallet.batchRestore(100, 50, 0);
