...
test("handles bulk subscribers with filtering and CSV export", async ({
  page,
}) => {
  await page.evaluate(() => {
  ...
  const counts = await page.evaluate((freq) => {
    const subs = ((window as any).subs as any[]).filter(
      (s) => s.frequency === freq
    );
    const container = document.querySelector("div")!;
    for (const child of Array.from(container.children)) {
      const idx = parseInt(child.textContent || "0", 10) - 1;
      if (((window as any).subs as any[])[idx].frequency !== freq)
        child.remove();
    }
    ...
    return {
      ...
      csvRows: csv.split("\n").length - 1,
      shown: container.children.length,
    };
  }, frequency);
