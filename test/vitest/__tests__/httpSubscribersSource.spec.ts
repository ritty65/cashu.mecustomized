describe('httpSubscribersSource', () => {
  ...
  beforeEach(() => { process.env = { ...OLD_ENV }; vi.resetModules(); });
  afterAll(() => { process.env = OLD_ENV; });

  it('throws when endpoint missing', async () => {
    delete process.env.VITE_SUBSCRIBERS_ENDPOINT;
    const mod = await import('src/data/httpSubscribersSource');
    await expect(mod.fetchSubscribers()).rejects.toMatchObject({ code: 'SUBSCRIBERS_ENDPOINT_MISSING' });
  });

  it('fetches and normalizes array', async () => {
    process.env.VITE_SUBSCRIBERS_ENDPOINT = 'https://example.com/subs';
    // @ts-ignore
